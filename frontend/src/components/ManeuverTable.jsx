...
export default function ManeuverTable({ analysis, advisory, selectedId, onSelect }) {
  ...
  return (
    <div>
      {/* Legend */}
      <p style={{ fontSize: '0.62rem', color: 'var(--muted)', marginBottom: '0.5rem' }}>
        <span style={{ color: 'var(--accent)' }}>Backend values</span>
        {' '}are authoritative.{' '}
        <span style={{ color: 'var(--purple)' }}>Granite ranks</span>
        ...
        >
          ...
          <thead>
            <tr>
              ...
              <th scope="col" style={{ color: 'var(--purple)' }}>Granite Rank</th>
            </tr>
          </thead>
          <tbody>
            {analysis.candidates.map(c => {
              ...
              return (
                ...
                >
                  ...
                  <td>{c.label}</td>
                  <td style={{ fontFamily: 'var(--font-mono)' }}>{fmt(c.delta_v_ms, 2)}</td>
                  ...
                  <td>
                    {c.is_safe
                      ? <span className="safe-tag" aria-label="Safe">✓ SAFE</span>
                      : (
                        <span
                          className="unsafe-tag"
                  ...
                  <td>
                    {r
                      ? <span className="rank-tag" aria-label={`Granite rank ${r.rank}`}>#{r.rank}</span>
              ...
              <div className="cand-card-header">
                ...
                <span>
                  {c.is_safe
                    ...
                    : <span className="unsafe-tag">✗ REJECTED</span>
                  }
                </span>
              </div>
              <div className="cand-card-label">{c.label}</div>
              <div className="cand-card-meta" style={{ marginTop: '0.4rem' }}>
                ...
                {r && (
                  <div className="cand-meta-kv">
                    ...
                    <span className="v" style={{ color: 'var(--purple)' }}>#{r.rank}</span>
                  </div>
                )}
              </div>
            </div>
      ...
      {selectedId && rankMap[selectedId] && (
        <div className="cand-explanation" role="region" aria-label="Granite explanation for selected candidate">
          <strong>Granite explanation for {selectedId}:</strong>{' '}
          {rankMap[selectedId].explanation}
          <div style={{ marginTop: '0.3rem', fontSize: '0.62rem', fontStyle: 'italic', color: 'var(--muted)' }}>
